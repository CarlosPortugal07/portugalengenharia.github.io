...
export default function Proposta() {
  ...
  const estimate = useMemo(() => {
    ...
    const monthlyKwh = conta / TARIFFA_KWH;
    ...
    return { hasInput: true, realKwp, theoreticalKwp, monthlyKwh, modules, monthlySavings, annualSavings, investment, payback: investment / Math.max(annualSavings, 1) };
  }, [conta]);
    ...
    <div className="proposal-page">
      ...
      <main className="proposal-container">
        <section className="proposal-card proposal-input-card">
          ...
          <p className="proposal-muted">Informe apenas o valor médio da sua conta e veja uma primeira faixa de investimento em poucos segundos. O consumo em kWh é estimado internamente.</p>
        ...
        <section>
          <div className="proposal-result"><h2>{estimate.hasInput ? "Sistema recomendado para você" : "Informe o valor da sua conta para simular"}</h2><p className="proposal-small">Tarifa: R$ {TARIFFA_KWH.toFixed(2).replace(".", ",")}/kWh • HSP: {HSP_DAILY.toFixed(3).replace(".", ",")} h/dia • Perdas: {SYSTEM_LOSSES * 100}%</p><div className="proposal-kpis"><div><span>Potência</span><b>{estimate.hasInput ? `${estimate.realKwp.toFixed(2)} kWp` : "-"}</b><small>{estimate.hasInput ? `${estimate.modules}x ${MODULE_LABEL}` : "Aguardando conta"}</small></div><div><span>Consumo estimado</span><b>{estimate.hasInput ? `${estimate.monthlyKwh.toFixed(0)} kWh` : "-"}</b><small>{estimate.hasInput ? "por mês" : "Calculado pela tarifa"}</small></div><div><span>Investimento</span><b>{estimate.hasInput ? brl(estimate.investment) : "-"}</b><small>{estimate.hasInput ? "Financiamento 60x disponível" : "Aguardando conta"}</small></div><div><span>Payback</span><b>{estimate.hasInput ? `${estimate.payback.toFixed(1)} anos` : "-"}</b><small>{estimate.hasInput ? `${(estimate.realKwp * 1.2).toFixed(1)}t CO₂/ano evitado` : "Aguardando conta"}</small></div></div></div>
