...
export default function Proposta() {
  ...
  const [conta, setConta] = useState(850);
  const [consumo, setConsumo] = useState(650);
  ...
  const estimate = useMemo(() => {
    ...
    return { realKwp, modules, monthlySavings, annualSavings, investment, payback: investment / Math.max(annualSavings, 1) };
  }, [conta, consumo]);
  ...
  return (
    <div className="proposal-page">
      ...
      <main className="proposal-container">
        ...
        <section>
          <div className="proposal-result"><h2>Sistema recomendado para você</h2><p className="proposal-small">Cálculo com irradiação de Valinhos 5,2 kWh/m²/dia • Perdas 20% • Kit Jinko 575W + Solis</p><div className="proposal-kpis"><div><span>Potência</span><b>{estimate.realKwp.toFixed(2)} kWp</b><small>{estimate.modules}x 575W • 1 string</small></div><div><span>Economia mensal</span><b>{brl(estimate.monthlySavings)}</b><small>{brl(estimate.annualSavings)}/ano</small></div><div><span>Investimento</span><b>{brl(estimate.investment)}</b><small>Financiamento 60x disponível</small></div><div><span>Payback</span><b>{estimate.payback.toFixed(1)} anos</b><small>{(estimate.realKwp * 1.2).toFixed(1)}t CO₂/ano evitado</small></div></div></div>
          <div className="proposal-commercial"><div className="proposal-commercial-title"><h2>Proposta Comercial</h2><span>PRÉVIA TÉCNICA</span></div><p>Cliente: <b>{nome || "-"}</b> • {cidade || "Valinhos-SP"} • {whatsapp || "-"}</p><p><b>Sistema:</b> {estimate.realKwp.toFixed(2)} kWp — {estimate.modules} módulos Jinko 575W + inversor {Math.ceil(estimate.realKwp)} kW — {tipo}</p><div className="proposal-tags"><span>JINKO 575W N-TYPE</span><span>SOLIS / DEYE</span><span>ESTRUTURA AL</span><span>STRING BOX + DPS</span><span>CABO SOLAR 6mm²</span></div><div className="proposal-benefits"><div><b>Garantias</b><br />25 anos painéis* • 10 anos inversor • 1 ano instalação • Monitoramento</div><div><b>Incluso</b><br />Projeto + ART + Homologação CPFL + Instalação + Comissionamento</div></div><p className="proposal-footnote">*Condições conforme fabricante e escopo final. A proposta definitiva depende da análise da fatura, localização, telhado, sombreamento, padrão de entrada e vistoria técnica.</p><button className="proposal-whatsapp" type="button" onClick={sendToWhatsApp}><MessageCircle size={18} /> Falar com a engenharia pelo WhatsApp</button></div>
